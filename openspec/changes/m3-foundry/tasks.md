...
- [ ] 1.1 Close the `-ip :PORT` unknown: determine whether trs80gp delivers
...
- [ ] 1.2 Add a checksum-pinned trs80gp fetch to `tools/`: download
...
- [ ] 1.3 Mirror the pinned archive somewhere the project controls, since
  upstream keeps only the last few releases online, and record where.

## 2. Server (foundry, store, reconciliation)

- [ ] 2.1 Define the config block v1 format in one place on the Python side —
...
- [ ] 2.2 Add the profile store: `server/machines/<id>.json` with the schema
...
- [ ] 2.3 Add machine-ID assignment from `server/machines/.next-id` starting at
...
- [ ] 2.4 Implement `server/foundry.py` with `new`, `list`, `show`, `mint <id>`.
...
- [ ] 2.5 Teach the server `--machines-dir` (default `server/machines`), move
...
- [ ] 2.6 Implement HELLO reconciliation in one function with its own tests:
  ...
  profile.
- [ ] 2.7 Prove the server never mutates identity, drive map, link config, or
...
- [ ] 3.1 Reserve the block: add `machine/config.inc` with `CFGBLK` and the
...
- [ ] 3.2 Read and validate the block at cold boot — magic, version, checksum
  under the existing wire-checksum routine — before the wire is touched.
- [ ] 3.3 On a valid block, copy the machine ID into the HELLO payload and
...
- [ ] 3.4 On a valid block, preload `DMAP`/`MAPCNT` from the block's cached map
...
- [ ] 3.5 On an invalid, unstamped, or unknown-version block, print the honest
  "config block unreadable" line, skip HELLO entirely, and land at the prompt
  in Local-Only Mode.
- [ ] 3.6 Extend `config` to report the block: validity, format version, and
...
- [ ] 3.7 Rebuild strict-8080 and confirm every existing harness scenario still
  passes against a *minted* 1001 image.

## 4. Machine — TRS-80 Model 4 template

- [ ] 4.1 Prove the relocation in isolation, before any wire work: a template
  ...
  proof.
- [ ] 4.2 Lay down the CP/M page-zero vectors after the map switch and confirm
  the TPA boundary, so `0005` and the restart vectors exist before anything
  uses them.
- [ ] 4.3 Bring up the TR1865 wire at `E8`–`EB` with the verified init
  (`OUT (E8h),0`; `0EEh`→`E9` for 9600 or `0FFh` for 19200; `6Fh`→`EA`), and
  mirror every console character to the printer port at `F8h`.
- [ ] 4.4 Place the config block in the Model 4 template at the same offset from
...
- [ ] 4.5 Port the boot ladder: block read and validation, DMAP preload, HELLO
  with the bounded retry loop that survives the dropped first byte, banner,
  prompt, and Local-Only Mode with no server.
- [ ] 4.6 Wire the console input channel chosen in 1.1 so the prompt can be
  driven, and confirm `config` and `ls /dev` are reachable on this platform.

## 5. Harness

- [ ] 5.1 Add a trs80gp scenario runner alongside the SIMH one: launch with
...
- [ ] 5.2 Give scenarios a temporary machines directory and a foundry call, so
  a scenario can mint its own images instead of depending on committed ones.
- [ ] 5.3 Add the two-machines scenario: mint 1001 (Altair) and 1002 (Model 4)
...
- [ ] 5.4 Add the probe-loop scenario: profile with unknown RAM → mint → boot →
...
- [ ] 6.2 Update `README.md`'s proof section with the M3 scenarios and the
  two-platform story, including how to fetch the pinned trs80gp binary.
- [ ] 6.3 Fold the 1.1 answer and any Model 4 surprises back into
