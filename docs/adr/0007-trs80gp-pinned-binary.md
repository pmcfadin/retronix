...
whether `-ip :PORT` restores keystroke injection is unverified and is the
first question M3 must close, with ROM-owned console input over the tap as
the fallback. trs80gp is not headless — it opens a window, so a CI runner
